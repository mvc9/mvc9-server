...
buffer.intervalClear = setInterval(() => {
  ...
  Object.keys(bufferMemory).map((path) => {
    if (!bufferMemory[path].data || bufferMemory[path].expireTime < currentTime) {
      ...
      logOnConsole({ name: 'Buffer', content: `Clear expired buffer "${path}"`, logLevel: 6 });
    }
  });
}, (config.server.BufferTime || 10000) + 1000);
...
buffer.readRes = (path, charset, alwaysReload, lifeTime) => {
  ...
  const loadFile = (path) => {
    ...
    if (isResExist) {
      ...
      logOnConsole({ name: 'Buffer', content: `Read form file "${path}"`, logLevel: 6 });
    }
    return {
  ...
  if (bufferMemory[path] && !alwaysReload) {
    if (bufferMemory[path].expireTime < (new Date()).getTime()) {
      ...
      logOnConsole({ name: 'Buffer', content: `Read form buffer "${path}"`, logLevel: 6 });
    }
  } else {
    bufferMemory[path] = loadFile(path);
  }
...
buffer.writeMem = (path, data, lifeTime) => {
  ...
  if (lifeTime) {
    ...
    logOnConsole({ name: 'Buffer', content: `Write to buffer "${path}"`, logLevel: 6 });
  }
  return data;
}
